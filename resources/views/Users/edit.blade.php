<x-app-layout>
  <div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div class="mb-6">
      <h1 class="text-base font-semibold text-gray-800">Update User</h1>
      <p class="text-xs text-gray-400 mt-0.5">Update data pengguna</p>
    </div>

    <form method="POST" action="{{ route('users.update', $user->id_user) }}" id="form"
        ...
        <select id="role" name="role" required
          ...
          @foreach ($roles as $role)
            <option value="{{ $role->id_role }}"
              {{ (old('role') ?? $user->role_id) == $role->id_role ? 'selected' : '' }}>
              {{ $role->name }}
            </option>
          @endforeach
        </select>

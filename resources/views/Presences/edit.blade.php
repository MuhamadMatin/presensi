<x-app-layout>
  <div class="max-w-5xl mx-auto py-8 px-4">
    <div class="mb-6">
      <h1 class="text-base font-semibold text-gray-800">Update Presensi</h1>
      <p class="text-xs text-gray-400 mt-0.5">Update data presensi</p>
    </div>

    <form method="POST" action="{{ route('presences.update', $presence->id_presence) }}" id="form"
      ...
      <div class="w-full col-span-2 flex flex-col md:flex-row gap-4">
        {{-- masuk --}}
        <div class="w-full">
          <label for="masuk" class="block mb-1.5 text-xs font-semibold text-gray-400 uppercase tracking-widest">
            masuk
          </label>
          <input type="datetime-local" id="masuk" name="masuk" value="{{ old('masuk') ?? $presence->entry }}"
        ...
        {{-- keluar --}}
        <div class="w-full">
          <label for="keluar" class="block mb-1.5 text-xs font-semibold text-gray-400 uppercase tracking-widest">
            Keluar
          </label>
          <input type="datetime-local" id="keluar" name="keluar" value="{{ old('keluar') ?? $presence->out }}"
            class="w-full px-3.5 py-2.5 text-sm text-gray-800 bg-white border rounded-lg focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition duration-150 border-gray-400 cursor-pointer" />
          @error('keluar')
            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
          @enderror
      ...
      <div class="w-full col-span-2 flex flex-col md:flex-row gap-4">
          ...
          <select id="status" name="status" required
            ...
            @foreach ($statuses as $status)
              <option value="{{ $status->id_status }}"
                {{ (old('status') ?? $presence->status_id) === $status->id_status ? 'selected' : '' }}>
                {{ $status->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="w-full">
          ...
          <div class="relative">
            ...
            <input type="text" id="lokasi" name="lokasi" value="{{ old('lokasi') ?? $presence->location }}"
              placeholder="DSM"
      ...
      <div class="col-span-2">
        ...
        <textarea id="deskripsi" name="deskripsi" rows="5" placeholder="Tambahkan catatan jika diperlukan..."
          class="w-full px-3.5 py-2.5 text-sm text-gray-800 bg-white border border-gray-400 rounded-lg placeholder-gray-300 resize-none focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition duration-150">{{ old('deskripsi') ?? $presence->description }}</textarea>
        @error('deskripsi')
          <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="col-span-2">
        ...
        <div id="foto-preview" class="flex gap-2 mt-2.5 flex-wrap">
          @foreach ($presence->images as $images)
            <img class="w-20 h-20 object-cover rounded-lg border border-gray-200"
              src="{{ Storage::url($images->image) }}" alt="foto presensi" />
          @endforeach
        </div>
      </div>
